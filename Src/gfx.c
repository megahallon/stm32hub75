...
static uint16_t textcolor = 0xfff;
static uint16_t textbgcolor = 0x000;
...
{
	if((x >= MATRIX_PANEL_WIDTH) || // Clip right
	   (y >= MATRIX_PANEL_HEIGHT) || // Clip bottom
	...
	if(bg != color) { // If opaque, draw vertical line for last column
	    ...
	    else          drawFillRect(x+5*size, y, size, 8*size, bg);
	}
...
void drawString(const char *str) {
  for (int i = 0; i < strlen(str); ++i) {
    ...
    if(c == '\n') {                        // Newline?
        ...
        if(wrap && ((cursor_x + textsize * 6) > MATRIX_PANEL_WIDTH)) { // Off right?
            cursor_x  = 0;                 // Reset x to zero,
            cursor_y += textsize * 8;      // advance y one line
        }
